...
*/
    ...
    if (codec_ctx) {
        ...
        if (h->type == NGX_RTMP_MSG_AUDIO) {
           ...
           if ( ( codec_ctx->video_codec_id == NGX_RTMP_VIDEO_H264 && ngx_rtmp_is_codec_header(in) ) ||
                ( codec_ctx->video_codec_id == NGX_RTMP_VIDEO_H265 && ngx_rtmp_is_hevc_codec_header(in) )
              ) 
            {
                prio = 0;
                mandatory = 1;
